...
-- any complexity. The following code uses a tying-the-know data passing
...
-- | _O(N) and O(N) calls to the predicate._ Compute the list of Z-function
...
-- | _O(N) and O(N) plus-1's._ Compute the list of Z-function states using a
-- given number type.
...
-- | _O(N), O(N) calls to the predicate, and O(N) plus-1's._ Compute the list of
